...
class Agent(object):
    def __init__(self, case_info, role_name, simulation_id, role_info=None, memory_manager=None):
...
4.你的回复应该是纯文本格式'''
        ...
        # 构建记忆上下文：有记忆管理器时使用结构化记忆，不再直接塞入原始 history
        if self.memory_manager:
            history_text = self.memory_manager.build_memory_prompt()
        else:
        ...
        if summary:
            ...
            # 记录发言到记忆管理器（已简化为 CourtFlow 中的 update_round_summary）
            # if self.memory_manager:
            #     self.memory_manager.add_speech(self.role, self.name, response, 'general')
            process_result = process_court_internal({
                'simulation_id': self.simulation_id,
                'role': self.role_key,
                'role_desc': self.role,
...
5.你的回复应该是纯文本格式'''
        ...
        # 记录发言到记忆管理器（已简化为 CourtFlow 中的 update_round_summary）
        # if self.memory_manager:
        #     self.memory_manager.add_speech(self.role, self.name, response, 'debate')
        process_result = process_court_internal({
                'simulation_id': self.simulation_id,
                'role': self.role_key,
                'role_desc': self.role,
...
5.你的回复应该是纯文本格式'''
            ...
            # 构建记忆上下文（对方视角）：有记忆管理器时使用结构化记忆，不再直接塞入原始 history
            if obj.memory_manager:
                history_text_obj = obj.memory_manager.build_memory_prompt()
            else:
                history_text_obj = history['text']
            ...
            # 记录对方发言到记忆管理器（已简化为 CourtFlow 中的 update_round_summary）
            # if obj.memory_manager:
            #     obj.memory_manager.add_speech(obj.role, obj.name, response, 'debate')
            # 将数据发送到服务器
            process_result = process_court_internal({
