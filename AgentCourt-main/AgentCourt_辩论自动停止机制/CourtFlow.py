...
from memory import CourtMemoryManager
import os
from collections import Counter

# 你的自定义输出根目录（改为自己的路径）
OUTPUT_ROOT = r"D:\AgentCourt-main\output"
...
def flow(judge, left_lawyer, right_lawyer, case, role_data, simulation_id, stop_event=None):
    ...
    # 每个案件一个独立文件夹
    case_output_dir = os.path.join(OUTPUT_ROOT, simulation_id)
    os.makedirs(case_output_dir, exist_ok=True)

    # 初始化记忆管理器
    memory_manager = CourtMemoryManager(
        case_info=case_info,
        simulation_id=simulation_id,
        output_folder=case_output_dir,
    )

    # 初始化案件基础信息和角色信息（基础层，庭审开始即填充）
    memory_manager.init_case_info(
        judge_info=judge,
        left_lawyer_info=left_lawyer,
        right_lawyer_info=right_lawyer,
    )

    judge_agent = Agent(case_info=case_info, role_name='judge', role_info=judge, simulation_id=simulation_id, memory_manager=memory_manager)
    if role_data['role'] == 'robot':
        left_lawyer_agent = Agent(case_info=case_info, role_info=left_lawyer,
                                  role_name='left_lawyer', simulation_id=simulation_id, memory_manager=memory_manager)
        right_lawyer_agent = Agent(case_info=case_info, role_info=right_lawyer,
                                   role_name='right_lawyer', simulation_id=simulation_id, memory_manager=memory_manager)

    clerk_agent = Agent(case_info=case_info, role_name='clerk', simulation_id=simulation_id, memory_manager=memory_manager)
    left_agent = Agent(case_info=case_info, role_name='left', simulation_id=simulation_id, memory_manager=memory_manager)
    right_agent = Agent(case_info=case_info, role_name='right', simulation_id=simulation_id, memory_manager=memory_manager)
    ...
    # 书记员宣布纪律
    round_counter += 1
    memory_manager.set_phase("书记员宣布纪律")
    memory_manager.set_current_task("宣布法庭纪律，交代庭前准备")
    ...
    _update_round(memory_manager, history, round_counter, "书记员")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return

    # 法官宣布开庭，并核对当事人身份信息和代理人代理权限
    round_counter += 1
    memory_manager.set_phase("法官宣布开庭")
    memory_manager.set_current_task("宣布开庭，核对当事人身份，要求原告陈述")
    ...
    _update_round(memory_manager, history, round_counter, "法官")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 原告陈述个人信息
    round_counter += 1
    memory_manager.set_phase("原告陈述身份")
    memory_manager.set_current_task("陈述身份信息")
    ...
    _update_round(memory_manager, history, round_counter, "原告")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 被告陈述个人信息
    round_counter += 1
    memory_manager.set_phase("被告陈述身份")
    memory_manager.set_current_task("陈述身份信息")
    ...
    _update_round(memory_manager, history, round_counter, "被告")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 法官询问原告对身份有无异议
    round_counter += 1
    memory_manager.set_phase("法官询问身份异议")
    memory_manager.set_current_task("询问原告对身份有无异议")
    ...
    _update_round(memory_manager, history, round_counter, "法官-原告")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    round_counter += 1
    memory_manager.set_phase("法官询问身份异议")
    memory_manager.set_current_task("询问被告对身份有无异议")
    ...
    _update_round(memory_manager, history, round_counter, "法官-被告")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    round_counter += 1
    memory_manager.set_phase("法官告知权利义务")
    memory_manager.set_current_task("告知诉讼权利义务，询问是否申请回避")
    ...
    # 原告表明听清楚了且不申请回避
    round_counter += 1
    memory_manager.set_phase("原告回应权利义务")
    memory_manager.set_current_task("表明听清楚了且不申请回避")
    ...
    # 被告表面听清楚了且不申请回避
    round_counter += 1
    memory_manager.set_phase("被告回应权利义务")
    memory_manager.set_current_task("表明听清楚了且不申请回避")
    ...
    # 原告宣读起诉书
    round_counter += 1
    memory_manager.set_phase("法庭调查-原告陈述诉请")
    memory_manager.set_current_task("说明诉讼请求，阐明事实和理由")
    ...
    # 被告进行答辩
    round_counter += 1
    memory_manager.set_phase("法庭调查-被告答辩")
    memory_manager.set_current_task("针对原告的诉讼请求发表答辩意见")
    ...
    # 原告举证
    round_counter += 1
    memory_manager.set_phase("法庭调查-原告举证")
    memory_manager.set_current_task("针对诉讼请求、事实和理由进行举证")
    ...
    _update_round(memory_manager, history, round_counter, "原告律师")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 被告质证
    round_counter += 1
    memory_manager.set_phase("法庭调查-被告质证原告证据")
    memory_manager.set_current_task("对原告证据逐个发表质证意见")
    ...
    _update_round(memory_manager, history, round_counter, "被告律师")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 被告举证
    round_counter += 1
    memory_manager.set_phase("法庭调查-被告举证")
    memory_manager.set_current_task("针对答辩意见进行举证")
    ...
    # 原告质证
    round_counter += 1
    memory_manager.set_phase("法庭调查-原告质证被告证据")
    memory_manager.set_current_task("对被告证据逐个发表质证意见")
    ...
    # 原告补充
    round_counter += 1
    memory_manager.set_phase("法庭调查-询问原告是否补充")
    memory_manager.set_current_task("询问原告是否需要发问或补充说明")
    ...
    _update_round(memory_manager, history, round_counter, "法官-原告律师")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 被告补充
    round_counter += 1
    memory_manager.set_phase("法庭调查-询问被告是否补充")
    memory_manager.set_current_task("询问被告是否需要发问或补充说明")
    ...
    _update_round(memory_manager, history, round_counter, "法官-被告律师")
    if stop_event and stop_event.is_set():
        print(f'Tread {simulation_id} Stopped')
        output(f'{case_output_dir}/history_{simulation_id}.json', history)
        return
    ...
    # 法庭调查结束
    round_counter += 1
    memory_manager.set_phase("法庭调查结束")
    memory_manager.set_current_task("宣布法庭调查结束")
    ...
    # 法官归纳争议焦点
    round_counter += 1
    memory_manager.set_phase("法庭辩论-法官归纳争议焦点")
    memory_manager.set_current_task("总结争议焦点，宣布法庭辩论开始")
    ...
    # 原告发表辩论意见
    round_counter += 1
    memory_manager.set_phase("法庭辩论-原告发表辩论意见")
    memory_manager.set_current_task("围绕争议焦点发表辩论意见")
    ...
    # 被告发表辩论意见
    round_counter += 1
    memory_manager.set_phase("法庭辩论-被告发表辩论意见")
    memory_manager.set_current_task("围绕争议焦点发表辩论意见")
    ...
    for i in range(Config.MAX_TURN):
        round_counter += 1
        memory_manager.set_phase(f"自由辩论-第{i + 1}轮")
        memory_manager.set_current_task("第{i + 1}轮辩论发言")
        result = left_lawyer_agent.ask(
            task='若本轮有新意见则发表辩论意见，请确保内容不与在法庭调查阶段已经发表过的意见重复，且不重复援引相同的法律依据或立场观点。辩论的目的是向法庭说明为何应支持我的诉求，而对方的答辩应当被驳回。辩论时可以从以下角度展开：第一，围绕本案的争议焦点发表辩论内容，本案的争议焦点不局限于法官总结的争议焦点，自己也可以思考是否还有其他没有被提到的争议焦点；第二，在围绕争议焦点发表意见时，要判断对方是否隐瞒一些证据或者，存在的虚假陈述内容或者证据，或者对方有哪些陈述矛盾的地方来反驳对方陈述的事实和证据；第三，在驳斥对方陈述的事实和证据时，要顺带提出或主张自己认为的法律事实以及自己的证据为何能够充分支撑自己的诉求；第四，主张本案应从何种角度裁判是合情合理的，引导以下法官的思维；第五，法律条文如何正确理解，判断对方对引述的法律规范的理解是否正确，同时积极援引先行的法律法规、部门规章、司法解释或者类案判决来增强自己的辩论说服力，以支持自己的诉求。若本轮无新意见，必须回复"无补充辩论意见<stop>"。',
            obj=right_lawyer_agent,
            task_obj='若本轮有新意见则发表辩论意见，请确保内容不与在法庭调查阶段已经发表过的意见重复，且不重复援引相同的法律依据或立场观点。辩论的目的是向法庭说明为何应支持我的诉求，而对方的答辩应当被驳回。辩论时可以从以下角度展开：第一，围绕本案的争议焦点发表辩论内容，本案的争议焦点不局限于法官总结的争议焦点，自己也可以思考是否还有其他没有被提到的争议焦点；第二，在围绕争议焦点发表意见时，要判断对方是否隐瞒一些证据或者，存在的虚假陈述内容或者证据，或者对方有哪些陈述矛盾的地方来反驳对方陈述的事实和证据；第三，在驳斥对方陈述的事实和证据时，要顺带提出或主张自己认为的法律事实以及自己的证据为何能够充分支撑自己的诉求；第四，主张本案应从何种角度裁判是合情合理的，引导以下法官的思维；第五，法律条文如何正确理解，判断对方对引述的法律规范的理解是否正确，同时积极援引先行的法律法规、部门规章、司法解释或者类案判决来增强自己的辩论说服力，以支持自己的诉求。若本轮无新意见，必须回复"无补充辩论意见<stop>"。',
            history=history, stop_event=stop_event, round_counter=i
        )

        # 每轮结束后提炼本轮核心，更新长期记忆
        _update_round(memory_manager, history, round_counter, "自由辩论")
    ...
    # 法官询问原告是否需要补充
    round_counter += 1
    memory_manager.set_phase("法庭辩论-询问原告补充")
    memory_manager.set_current_task("询问原告是否需要补充辩论意见")
    ...
    if flag:
        round_counter += 1
        memory_manager.set_phase("原告补充辩论意见")
        memory_manager.set_current_task("发表补充辩论意见")
    ...
    # 法官询问被告是否需要补充
    round_counter += 1
    memory_manager.set_phase("法庭辩论-询问被告补充")
    memory_manager.set_current_task("询问被告是否需要补充辩论意见")
    ...
    if flag:
        round_counter += 1
        memory_manager.set_phase("被告补充辩论意见")
        memory_manager.set_current_task("发表补充辩论意见")
    ...
    # 原告作最后陈述
    round_counter += 1
    memory_manager.set_phase("最后陈述-原告")
    memory_manager.set_current_task("原告作最后陈述")
    ...
    # 被告作最后陈述
    round_counter += 1
    memory_manager.set_phase("最后陈述-被告")
    memory_manager.set_current_task("被告作最后陈述")
    ...
    for idx in range(3):
        round_counter += 1
        memory_manager.set_phase(f"法庭宣判-第{idx + 1}次")
        memory_manager.set_current_task("宣布判决结果")
        temp_history = {
    ...
    # 出具判决书
    # round_counter += 1
    # memory_manager.set_phase("出具判决书")
    # memory_manager.set_current_task("撰写书面判决书")
    ...
    # 输出文件汇总
    print(f"[保存] 案件 {simulation_id} 输出目录: {case_output_dir}")
    for fname in os.listdir(case_output_dir):
        print(f"  - {fname}")

    # 替换 HTTP 回调为本地日志记录
    log_message = f"Simulation {simulation_id} Finished"
    log_file = f"{case_output_dir}/simulation_log.txt"
    with open(log_file, 'a', encoding='utf-8') as log:
        log.write(log_message + '\n')
    print(log_message)
    output(f'{case_output_dir}/history_{simulation_id}_complete.json', history)
    with open(f'{case_output_dir}/history_{simulation_id}_complete.txt', 'w', encoding='utf-8') as file:
        file.write(history['text'])


def _update_round(memory_manager, history, round_number, speaker):
    """辅助函数：提取本轮对话内容并更新记忆"""
    content = ""
    ...
    # 自由辩论阶段：原告和被告各说一轮，合并双方发言
    if "自由辩论" in speaker and len(dataset) >= 2:
        # 取最后两条，分别是原告和被告的发言
        plaintiff_resp = dataset[-2].get('response', '')
        defendant_resp = dataset[-1].get('response', '')
        content = f"【原告律师】{plaintiff_resp}\n【被告律师】{defendant_resp}"
    else:
        # 其他阶段：取最后一条
        content = dataset[-1].get('response', '')

    memory_manager.update_round_summary(
